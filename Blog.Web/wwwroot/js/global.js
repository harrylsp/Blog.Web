...
layui.use(['element', 'layer', 'util', 'form'], function () {
    ...
    layui.util.fixbar({
        ...
        click: function (type) {
            if (type === 'bar1') {
                ...
                if (sear.test($('.blog-share').attr('class'))) {
                    ...
                    // bar的个数，bar1 永远在最顶端，然后是 bar2 最后是 top
                    var barHeight = parseInt($('.layui-fixbar').css('height'));

                    // 判断是否有其他的bar
                    if ($('.layui-fixbar-top').css('display') == 'list-item') {

                        var offsetHeight = 50 * (parseInt(barHeight / 50) - 1) + 15; // layui 的每个 bar高度为50px 分享div一开始设置了bottom 为 15px
                        $('.blog-share').css('bottom', parseInt(offsetHeight) + 'px');
                    }
        ...
        // 工具集中有元素隐藏时触发(针对 置顶 功能 修改util.js源码添加的回调方法)
        hideCallback: function () {

            var sear = new RegExp('layui-show');
            if (sear.test($('.blog-share').attr('class'))) {
                var bottom = parseInt($('.blog-share').css('bottom'));
                $('.blog-share').css('bottom', (bottom - 50) + 'px');
            }
            
        },
        // 工具集中有元素显示时触发(针对 置顶 功能 修改util.js源码添加的回调方法)
